...
static u16 * rpgNANDLUTable = NULL;//[TOTAL_BLOCKS]; // 这个表可以让 1G byte 的Nand查找而不需要切换区块
...
bool ndInitNAND()
{
	...
	lastZone = 0xff;
	srcIsFree = false;
	ioRpgSetDeviceStatus( 0x01, 0, 0, 0 );
...
void ndBuildLUT()
{
	...
	if( rpgNANDLUTable != NULL )
		_FAT_mem_free( rpgNANDLUTable );
	rpgNANDLUTable = (u16 *)_FAT_mem_allocate(TOTAL_BLOCKS * sizeof(u16));
	...
	for( u16 z = 0; z < TOTAL_ZONES; ++z ) { // 遍历8个zone
		...
		for( u16 b = 0; b < BLOCKS_PER_ZONE; ++b ) { // 每个zone 128 个block
			...
			// 这个page就是每个block的第一个page(2112) 的最后一个sub page(528)地址
			u32 pageAddr = (z * BLOCKS_PER_ZONE + b) * 64 * 2048; 

			// 读取该page的redundant data
...
u32 ndLog2Phy( u32 logicAddress, u32 * oldPhyAddress )
{
	...
	if( isRead ) { // 读取操作只需要简单的换算出块地址
		...
		// 
		u16 freeBlock = ndSearchFreeBlock( zone );
...
void ndReadPages( u32 addr, u32 pageCount, u8 * buffer )
{
	...
	for( u32 i = 0; i < pageCount; ++i ) {
		...
		lastPhyAddr = phyAddr;
		ioRpgMemCopy64( buffer, ndGlobalRWBuffer, 512 );
...
void ndFinishPartialCopy()
{
	...
	if( 0 == ndwr::remainPageCount ) {
		if( ndwr::oldBlockAddr != ndwr::blockAddr )
			dbg_printf("THERE COULD BE A MISS ERASE!!!!\n");
		return;
	}
...
void ndReplaceBlock( u32 oldBlockAddr )
{
	...
	oldBlockAddr &= (~0x1ffff);

	u16 block = (oldBlockAddr >> 17);
    u16 * p = &rpgNANDLUTable[block];
    bool eccRetry = (*p & cBLK_ECC);
    *p |= cBLK_ECC;


	u32 zone = (oldBlockAddr / BYTES_PER_BLOCK) / BLOCKS_PER_ZONE;
	u32 freeBlock = ndSearchFreeBlock( zone );
	u32 newBlockAddr = (zone * BLOCKS_PER_ZONE + freeBlock) << 17;


	ecc2bitError = false;
	dbg_printf("ndCopyPages called from ndReplaceBlock()\n");
	ndCopyPages( oldBlockAddr, newBlockAddr, (u8)256/* or 0, they are same*/ );

	// repeat ecc error or 2 bit ecc error, should mark the src block bad
	if( eccRetry || ecc2bitError ) {
		// mark src block bad
		u32 * f0data = (u32 *)_FAT_mem_allocate(528);
		for( size_t i = 0; i < 528 / 4; ++i ) {
			f0data[i] = 0x0f0f0f0f;
		}
		ioRpgWriteNand( oldBlockAddr + 512 * 3, f0data, 528 );
		_FAT_mem_free(f0data);
		// and rebuild LUT
		ndBuildLUT();
		dbg_printf( "BLOCK REPLACED %08x to %08x\n", oldBlockAddr, newBlockAddr );
		wait_press_b();
	}
}
